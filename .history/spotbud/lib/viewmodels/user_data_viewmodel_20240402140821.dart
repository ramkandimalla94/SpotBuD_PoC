...
class UserDataViewModel extends GetxController {
  ...
  Future<void> addCustomBodyPart(String userId, String bodyPart) async {
    try {
      ...
      List<dynamic> customMachines =
          userData.get('customMachines') ?? <dynamic>[];

      // Check if the body part already exists
      bool exists = customMachines.any(
        (machine) => machine['bodyPart'] == bodyPart,
      );

      if (!exists) {
        // Add the new body part to the customMachines array
        customMachines.add({'bodyPart': bodyPart, 'machineNames': []});

        // Update the customMachines array in Firestore
        await userDocRef.update({'customMachines': customMachines});
      }
    } catch (e) {
