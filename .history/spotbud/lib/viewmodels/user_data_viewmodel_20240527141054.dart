...
class UserDataViewModel extends GetxController {
  ...
  Future<void> saveRole() async {
    try {
      ...
      print('Error saving role data: $e');
    }
  }

  Future<void> savehasinitialData() async {
    try {
      User? user = _auth.currentUser;
      if (user != null) {
        String userId = user.uid;
        await _firestore.collection('data').doc(userId).update({
          'hasInitialData': hasInitialData.value,
        });
        print('Role saved successfully');
      }
    } catch (e) {
  ...
  void saveLifestyle() async {
    try {
      User? user = _auth.currentUser;
      if (user != null) {
        String userId = user.uid;
        await _firestore.collection('data').doc(userId).update({
          'lifestyle': lifestyle.value.index,
        });
        // Update observables
        print('Lifestyle saved successfully');
      }
    } catch (e) {
      print('Error saving lifestyle data: $e');
    }
  }
