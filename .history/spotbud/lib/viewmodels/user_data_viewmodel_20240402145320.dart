...
class UserDataViewModel extends GetxController {
  ...
  void addCustomBodyPart(String bodyPart) async {
    try {
      ...
      if (user != null) {
        ...
        if (!exists) {
          ...
          getBodyParts(); // Refresh body parts list
        } else {
  ...
  void removeCustomBodyPart(String bodyPart) async {
    try {
      ...
      // Filter out the body part to be removed
      List<dynamic> updatedCustomMachines = customMachines
          .where((machine) => machine['bodyPart'] != bodyPart)
          .toList();

      // Update the customMachines array in Firestore
      await userDocRef.update({'customMachine': updatedCustomMachines});
      getBodyParts(); // Refresh body parts list
    } catch (e) {
      print('Error removing custom body part: $e');
    }
  }
