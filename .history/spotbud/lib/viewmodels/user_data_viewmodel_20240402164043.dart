...
class UserDataViewModel extends GetxController {
  ...
  Future<void> addCustomMachine(String bodyPart, String machineName) async {
    try {
      String bodyPartC = bodyPart.substring(0, 1).toUpperCase() +
          bodyPart.substring(1).toLowerCase();
      String machineNameC = machineName.substring(0, 1).toUpperCase() +
          machineName.substring(1).toLowerCase();
      User? user = _auth.currentUser;
      if (user != null) {
        String userId = user.uid;
        ...
        if (exists) {
          // Add the new machine to the existing body part
          for (var machine in customMachines) {
            if (machine['bodyPart'] == bodyPartC) {
              (machine['machineName'] as List<dynamic>).add(machineNameC);
            }
          }
        } else {
          // Add a new entry for the body part and machine
          customMachines.add({
            'bodyPart': bodyPartC,
        ...
        print('Custom machine successfully added.');
      } else {
        print('User not logged in');
      }
    } catch (e) {
      print('Error adding custom machine: $e');
    }
  }
  ...
  Future<void> removeCustomMachine(String bodyPart, String machineName) async {
    try {
      User? user = _auth.currentUser;
      if (user != null) {
        String userId = user.uid;
        ...
        // Fetch current customBodyPart array
        DocumentSnapshot userData = await userDocRef.get();
        List<dynamic> customBodyPart =
            userData.get('customMachine') ?? <dynamic>[];

        // Iterate through each index of the customBodyPart array
        for (var index in customBodyPart) {
          // Check if the body part matches
          if (index['bodyPart'] == bodyPart) {
            // Remove the machine name from the machineNames list
            index['machineName'].remove(machineName);
            break; // Exit loop once the machine is removed
          }
        }

        // Update the customBodyPart array in Firestore
        await userDocRef.update({'customMachine': customBodyPart});
      ...
      print('Error removing custom machine: $e');
    }
  }
}
