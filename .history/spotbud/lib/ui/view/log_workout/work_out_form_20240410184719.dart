  ...
  void _openBodyPartSelection() async {
    ...
    if (result != null && result is Map<String, String>) {
      ...
      if (bodyPart != null && machine != null) {
        ...
        fetchLatestSetDetails(machine).then((map) {
          if (map != null) {
            // Set the fetched values as hint text for set 1
            controller.getSets(exercise).forEach((set) {
              set.reps = map['reps'] ?? 'Data Not Available';
              set.weight = map['weight'] ?? 'Data Not Available';
            });
          } else {
          ...
          // Handle error case
          print('Error fetching latest set details: $error');
          controller.addExercise(exercise);
        });
      }
    }
