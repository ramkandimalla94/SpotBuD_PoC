...
class _WorkoutLoggingFormState extends State<WorkoutLoggingForm> {
  ...
  void initState() {
    super.initState();
    // Check if there's any saved workout data
    if (_tempWorkoutData != null) {
      // Populate the form fields with the saved workout data
      _selectedDate = DateTime.parse(_tempWorkoutData!['date']);
      _selectedStartTime = TimeOfDay.fromDateTime(
          DateTime.parse(_tempWorkoutData!['startTime']));
      _selectedEndTime =
          TimeOfDay.fromDateTime(DateTime.parse(_tempWorkoutData!['endTime']));
      // You may need to populate other form fields similarly based on your data structure

      // Clear the saved workout data to avoid showing outdated data
      _tempWorkoutData = null;
    }
  }
