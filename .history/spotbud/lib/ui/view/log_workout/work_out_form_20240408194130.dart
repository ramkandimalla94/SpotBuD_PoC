...
class _WorkoutLoggingFormState extends State<WorkoutLoggingForm> {
  ...
  void _showExerciseHistoryDialog(ExerciseData exercise) async {
    if (exercise == null) {
      // Show a message if exercise data is invalid
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text('Invalid Exercise'),
            content: Text('Please select a valid exercise.'),
            actions: <Widget>[
              TextButton(
                child: Text('OK'),
                onPressed: () {
      ...
      String machineName = exercise.name.split(' - ').last;

      // Show dialog to display exercise history
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text('Exercise History - ${exercise.name}'),
            content: Container(
  ...
  if (result != null && result is Map<String, String>) {
    ...
    if (bodyPart != null && machine != null) {
      ...
      // Show the exercise history dialog for the added exercise if user ID is available
      if (userId != null) {
        _showExerciseHistoryDialog(exercise, userId);
      } else {
        print('User is not logged in');
        // Handle case where user is not logged in
      }
