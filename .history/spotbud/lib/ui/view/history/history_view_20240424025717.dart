...
class _HistoryViewState extends State<HistoryView> {
  ...
  Widget _buildMonthYearSelector(DateTime selectedMonth) {
    ...
    final maxMonth = DateTime(currentDate.year + 1, currentDate.month + 6);

    final initialMonthIndex = (selectedMonth.year - minMonth.year) * 12 +
        selectedMonth.month -
        minMonth.month;
      ...
      child: NotificationListener<ScrollNotification>(
        onNotification: (scrollNotification) {
          if (scrollNotification is ScrollEndNotification) {
            final newSelectedMonthIndex =
                (scrollController.offset / 50.0).round() + minMonth.month - 1;
            final newSelectedMonth = DateTime(
              minMonth.year + newSelectedMonthIndex ~/ 12,
              newSelectedMonthIndex % 12 + 1,
            );
            setState(() {
        ...
        child: ListView.builder(
          ...
          itemCount: (maxMonth.year - minMonth.year) * 12 +
              ...
              1,
          itemBuilder: (context, index) {
            final month = DateTime(minMonth.year + index ~/ 12, index % 12 + 1);
