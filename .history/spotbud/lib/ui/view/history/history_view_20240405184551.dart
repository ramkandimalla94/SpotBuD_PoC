  ...
  List<Widget> _buildBodyPartSegments(List<Map<String, dynamic>>? bodyParts) {
    ...
    return bodyParts.map((bodyPart) {
      ...
      if (exercises == null || exercises.isEmpty) {
        return Text('No exercises recorded for this body part.');
      }
      ...
      return Column(
        ...
        children: exercises.map<Widget>((exercise) {
          final name = exercise['exercisename'] as String? ??
              'Exercise Name not provided';
          final sets = exercise['sets'] as List<Map<String, dynamic>>?;
          if (sets == null || sets.isEmpty) {
            return Text('No sets recorded for $name.');
          }
          ...
          return Column(
            ...
            children: [
              Text('Exercise: $name'),
              ...sets.map((set) {
                final reps = set['reps'] as String? ?? 'Reps not provided';
                final weight =
                    set['weight'] as String? ?? 'Weight not provided';
                final notes = set['notes'] as String? ?? 'Notes not provided';
                return Padding(
