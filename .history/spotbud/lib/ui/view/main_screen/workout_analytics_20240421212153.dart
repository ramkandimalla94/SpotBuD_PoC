...
class _ExerciseAnalyticsScreenState extends State<ExerciseAnalyticsScreen> {
  ...
  Widget _buildBarGraph(List<Workout> workouts) {
    ...
    dateWeightsMap.forEach((key, value) => value.sort());

    // Create BarChartGroupData for each date
    List<BarChartGroupData> barChartGroups =
        dateWeightsMap.entries.map((entry) {
      DateTime date = entry.key;
      List<double> weights = entry.value;
      double sumWeights =
          weights.isNotEmpty ? weights.reduce((a, b) => a + b) : 0.0;
