...
class _ExerciseAnalyticsScreenState extends State<ExerciseAnalyticsScreen> {
          ...
          if (selectedExercise.isNotEmpty)
            Column(
              ...
              children: [
                ...
                SizedBox(height: 16.0),
                _buildWeightChart(workoutController.workouts, selectedExercise,
                    selectedOption),
              ],
            ),
        ],
  ...
  Widget _buildWeightChart(
    ...
    DateTime? minDate = weightData.isNotEmpty
        ? weightData.keys.reduce((a, b) => a.isBefore(b) ? a : b)
        : null;
    DateTime? maxDate = weightData.isNotEmpty
        ? weightData.keys.reduce((a, b) => a.isAfter(b) ? a : b)
        : null;

    // Fill missing dates with empty lists
    if (minDate != null && maxDate != null) {
      for (var date = minDate;
          date.isBefore(maxDate) || date.isAtSameMomentAs(maxDate);
          date = date.add(Duration(days: 1))) {
        weightData.putIfAbsent(date, () => []);
      }
    }
    ...
    weightData.forEach((date, weights) {
      ...
      switch (selectedOption) {
        case 'Min Weight':
          value = weights.isEmpty ? 0 : weights.reduce((a, b) => a < b ? a : b);
          break;
        case 'Max Weight':
          value = weights.isEmpty ? 0 : weights.reduce((a, b) => a > b ? a : b);
          break;
        case 'Average Weight':
        default:
          double sum = weights.isNotEmpty ? weights.reduce((a, b) => a + b) : 0;
          value = weights.isNotEmpty ? sum / weights.length : 0;
      }
    ...
    return Container(
      ...
      child: LineChart(
        LineChartData(
          ...
          minX: minDate?.day.toDouble() ?? 0,
          maxX: maxDate?.day.toDouble() ?? 0,
          minY: 0,
