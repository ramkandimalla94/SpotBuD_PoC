...
class _ExerciseAnalyticsScreenState extends State<ExerciseAnalyticsScreen> {
              ...
              children: [
                ...
                Column(
                  children: [
                    ...
                    _buildBarGraph(workoutController.workouts)
                  ],
                ),
              ],
            ),
  ...
  Widget _buildBarGraph(List<Workout> workouts) {
    // Create BarChartGroupData for each set
    List<BarChartGroupData> barChartGroups = [];

    // Iterate through workouts
    for (var workout in workouts) {
      DateTime date = DateTime.parse(workout.date);
      double xOffset = date.day.toDouble();
      ...
      for (var exercise in workout.exercises) {
        ...
        for (var set in exercise.sets) {
          ...
          // Create BarChartRodData for each set
          ...
          // Add the groupData to barChartGroups
          barChartGroups.add(groupData);

          // Increment xOffset for the next set
          xOffset += 1.0;
        }
      }
    }
    ...
    double maxY = barChartGroups.isNotEmpty
        ? barChartGroups
                .map((group) => group.barRods[0].toY.toDouble())
                .reduce((a, b) => a > b ? a : b) +
            10
        : 10;
