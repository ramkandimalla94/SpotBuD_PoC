...
class _ExerciseAnalyticsScreenState extends State<ExerciseAnalyticsScreen> {
    ...
    if (workoutController.workouts.isNotEmpty) {
      setState(() {
        selectedExercise = workoutController.workouts
            .expand((workout) => workout.exercises.map((exercise) => exercise.machine))
            .toSet()
            .toList()[0];
      });
    }
  ...
  Widget _buildExerciseAnalytics(String selectedExercise) {
    List<Workout> filteredWorkouts = workoutController.workouts.where((workout) {
      return workout.exercises.any((exercise) => exercise.machine == selectedExercise);
    }).toList();
