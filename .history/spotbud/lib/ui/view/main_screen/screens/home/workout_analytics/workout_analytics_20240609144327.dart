...
import 'package:spotbud/ui/widgets/chip_select.dart';
...
class _ExerciseAnalyticsScreenState extends State<ExerciseAnalyticsScreen> {
            ...
            if (showLineChart) ...[
              ...
              buildChipSelect(
                ...
                onOptionSelected: (option) {
                  ...
                  // Call _buildBarGraph with the selected option
                  _buildBarGraph(
                      workoutController.workouts, selectedExercise, option);
                },
                initialOption: 'Weight', // Optional initial option
              ),
            ],
          ],
        ],
  ...
  Widget _buildBarGraph(
    ...
    List<BarChartGroupData> barChartGroups = sortedDates.map((date) {
      List<double> values = selectedOption == 'Weight'
          ? dateWeightsMap[date]!
          : dateRepsMap[date]!.map((reps) => reps.toDouble()).toList();
      return BarChartGroupData(
        x: date
            .millisecondsSinceEpoch, // Using milliseconds since epoch as x value
        barRods: values.map((value) {
          return BarChartRodData(
            toY: value, // Set y value directly instead of toY
            color: Theme.of(context).colorScheme.primary,
          );
        }).toList(),
      );
    ...
    // Calculate maxY based on the maximum value
