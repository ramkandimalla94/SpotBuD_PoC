...
import 'package:spotbud/api/chatmodel.dart' as chat_model;
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  final List<chat_model.ChatMessage> _messages = [];
  ...
  void _loadMessages() {
    _chatService.getMessages().listen((messages) {
      setState(() {
        _messages.clear();
        _messages.addAll(messages.map((m) => m));
      });
    });
  }

  void _sendMessage(String message) async {
    final newMessage = chat_model.ChatMessage(
      text: message,
      isUser: true,
    ...
    await _chatService
        .saveMessage(newMessage.toMap() as chat_model.ChatMessage);
    try {
      ...
      setState(() {
        _isBotTyping = false;
        _messages.insert(0, botMessage as chat_model.ChatMessage);
      });
      await _chatService
          .saveMessage(botMessage.toMap() as chat_model.ChatMessage);
      _scrollToBottom();
    } catch (e) {
      setState(() {
        _isBotTyping = false;
