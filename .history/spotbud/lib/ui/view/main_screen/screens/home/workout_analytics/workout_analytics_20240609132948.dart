...
class _ExerciseAnalyticsScreenState extends State<ExerciseAnalyticsScreen> {
  ...
  weightData.forEach((date, weights) {
    ...
    weightLineChartSpots.add(FlSpot(date.millisecondsSinceEpoch.toDouble(), value));
  });
  ...
  repData.forEach((date, reps) {
    ...
    switch (selectedOption) {
      ...
      case 'Average Reps':
      default:
        int sum = reps.reduce((a, b) => a + b);
        value = sum ~/ reps.length;
    }
    repLineChartSpots.add(FlSpot(date.millisecondsSinceEpoch.toDouble(), value.toDouble()));
  });
